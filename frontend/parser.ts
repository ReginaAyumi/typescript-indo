...
export default class Parser {
	...
	private parse_for_loop(): Stmt {
		...
		this.expect(
		  TokenType.Semicolon,
		  "Expected ';' after initialization statement in for loop"
		);
		const condition = this.parse_expr();
		this.expect(
		  TokenType.Semicolon,
		  "Expected ';' after loop condition in for loop"
		);
		const increment = this.parse_stmt();
		this.expect(TokenType.CloseParen, "Expected ')' after for loop clauses");
	  
		this.expect(TokenType.OpenBrace, "Expected '{' before for loop body");
		const body: Stmt[] = [];
		while (
		  this.at().type !== TokenType.EOF &&
		  this.at().type !== TokenType.CloseBrace
	...
	private parse_if_stmt(): Stmt {
		...
		this.expect(TokenType.CloseBrace, "Expected '}' after if condition");
		...
		while (
			...
			if (this.at().type === TokenType.Else) {
				...
				this.expect(TokenType.OpenBrace, "Expected '{' after 'else' keyword");
				const elseBody: Stmt[] = [];
				while (
					this.at().type !== TokenType.EOF &&
					this.at().type !== TokenType.CloseBrace
				...
				this.expect(TokenType.OpenBrace, "Expected '{' after 'elif' keyword");
				const elifBody: Stmt[] = [];
				while (
					this.at().type !== TokenType.EOF &&
					this.at().type !== TokenType.CloseBrace
	...
	private parse_expr(): Expr {
		...
		while (this.at().type == TokenType.GTE || this.at().type == TokenType.LTE) {
			...
			if (this.at().type == TokenType.Equals) {
				throw "For GT-LS or equals that you use either '<=' or '>='"
			}
		}
		...
		while (
			...
			if (this.at().type == TokenType.Equals) {
				throw "When comparing varuables use '=='"
			}
		}
